...
describe("<ProblemSection />", () => {
  ...
  it('renders the main heading, subheading, and "Problema Común" badge', () => {
    ...
    expect(
      screen.getAllByTestId("icon-alert-triangle").length,
    ).toBeGreaterThanOrEqual(1);

    const mainHeading = screen.getByRole("heading", {
      level: 2,
      name: /¿Por qué tu CV no está generando entrevistas?/i,
    });
    ...
    expect(
      screen.getByText(
        /Muchos profesionales talentosos luchan por conseguir entrevistas debido a CVs mal optimizados/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it('renders the "Formato Inadecuado" card with its icon and description', () => {
    ...
    expect(
      screen.getByRole("heading", { level: 3, name: /Formato Inadecuado/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Los sistemas ATS rechazan CVs con formatos complejos o poco legibles/i,
      ),
    ).toBeInTheDocument();
  ...
  it('renders the "Falta de Palabras Clave" card with its icon and description', () => {
    ...
    expect(
      screen.getByRole("heading", {
        level: 3,
        name: /Falta de Palabras Clave/i,
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Sin las palabras clave correctas, tu CV nunca llegará a manos de un reclutador/i,
      ),
    ).toBeInTheDocument();
  ...
  it('renders the "Contenido Genérico" card with its icon and description', () => {
    ...
    expect(
      screen.getByRole("heading", { level: 3, name: /Contenido Genérico/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /CVs que no destacan logros específicos ni se adaptan al puesto deseado/i,
      ),
    ).toBeInTheDocument();
